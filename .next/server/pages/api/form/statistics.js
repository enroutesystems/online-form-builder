...
var firebaseConfig = __webpack_require__("fR0J");

// EXTERNAL MODULE: ./helpers/collections.js
var collections = __webpack_require__("cEQk");

// EXTERNAL MODULE: ./helpers/answer.js
var answer = __webpack_require__("iQSa");

// CONCATENATED MODULE: ./helpers/statistics.js
...
const getForms = async uid => {
  const forms = [];
  const snapshotForms = await firebaseConfig["b" /* firestore */].collection(collections["a" /* default */].forms).where('uid', '==', uid).get();
...
const formsStatistics = async uid => {
  const responses = [];
  const forms = await getForms(uid);

  for (let form of forms) {
    const formWithAnswers = await Object(answer["a" /* getResponses */])(form.id);
    responses.push(getUsersWhoAnsweredCount(formWithAnswers));
  }

  return responses;
};
// CONCATENATED MODULE: ./pages/api/form/statistics.js

/* harmony default export */ var statistics = __webpack_exports__["default"] = (async (req, res) => {
...
module.exports = __webpack_require__("+T32");
